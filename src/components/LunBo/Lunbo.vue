  ...
  <div class="box">
    <div @mouseenter="endInterval" @mouseleave="startInterval" class="container">
      <div :style="{width:wrap_width+'px',height:wrap_height+'px',left:offset_left+'px'}" class="slider-wrap">
        <div class='img' v-for="item in slider_des" :key="item.aid">
          <img :src="item.src" @click="goDetail(item.id)" >
        </div>
      </div>
      <div class="bottom">
        <ul class="pointContainer">
          <li @click="changeIndex(index)" :class="{point:true,active:nowIndex===index}" v-for="(point,index) in length" :key="point"></li>
        </ul>
      </div>
